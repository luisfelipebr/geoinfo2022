...
nuis <- polos_nuis %>%
  st_drop_geometry() %>%
  mutate(
    occupation_type = case_when(V5a == 1 ~ "11_Slum",
                                V5a == 2 ~ "12_Informal",
                                V5a == c(3,4,5,6,7,8) ~ "12_Other type"),
...
write_excel_csv2(results_table, "output/table2.csv")
write.csv2(round(prop.table(table(polos_nuis$V1, final1), margin = 1),2), "output/table3.csv")
write.csv2(round(prop.table(table(polos_nuis$V2b, final1), margin = 1),2), "output/table4.csv")
write.csv2(round(prop.table(table(nuis$occupation_type, final1), margin = 2),2), "output/table5.csv")
